...
import 'package:mechcalc/application/controllers/text_tool_controller.dart';

void main() {
  const controller = TextToolController();

  test('text tool output builds reusable copy text and status summaries', () {
    final output = controller.calculate(toolId: 'base64', input: 'MechCalc');
    ...
    final copyText = output.copyText(title: 'Base64', input: 'MechCalc');
...
''',
  ...
  test('base64 tool still detects plain text and encoded text', () {
    final encoded = controller.calculate(toolId: 'base64', input: 'MechCalc');
    final decoded =
    ...
    expect(decoded.primary, 'MechCalc');
  ...
  test('base64 tool supports data urls url safe payloads and wrapped text', () {
    ...
    expect(dataUrl.primary, 'Hello, MechCalc!');
...
''',
    ...
    expect(pem.primary, 'Hello, MechCalc!');
    expect(pem.insights, contains('已从 PEM 块中提取 Base64 payload 解码。'));

    expect(markdown.primary, 'MechCalc');
    expect(
      markdown.insights,
  ...
  test('json and csv tools return validation insights', () {
    final json = controller.calculate(
      toolId: 'json_format',
      input:
          '\ufeff{"name":"MechCalc","tools":["json","csv"],"meta":{"ok":true,"none":null}}',
    );
...
''',
  ...
  test('json formatter unwraps escaped json strings from logs', () {
    final escaped = controller.calculate(
      toolId: 'json_format',
      input: r'"{\"name\":\"MechCalc\",\"items\":[1,2],\"ok\":true}"',
    );
    final log = controller.calculate(
      toolId: 'json_format',
      input:
          r'INFO payload="{\"name\":\"MechCalc\",\"nested\":{\"ok\":true}}" status=200',
    );
    final singleLineJsonLike = controller.calculate(
      toolId: 'json_format',
      input: "payload = {name: 'MechCalc', flags: ['json',],};",
    );
...
''',
    ...
    final trailingLog = controller.calculate(
      toolId: 'json_format',
      input:
          '2026-06-09T08:00:00Z INFO payload={"name":"MechCalc","ok":true} status=200 duration_ms=8',
    );
    ...
    expect(markdownObject['name'], 'MechCalc');
    ...
    expect(trailingLogObject['name'], 'MechCalc');
...
''',
  ...
  test('jwt and invalid input expose actionable insights', () {
    ...
    expect(rich.detail, contains('iss: MechCalc'));
    ...
    expect(bearer.detail, contains('iss: MechCalc'));
  ...
  test('jwt decoder extracts tokens from url cookie and json fields', () {
    ...
    expect(cookie.detail, contains('iss: MechCalc'));
