# MechCalc Architecture

MechCalc 使用 Feature-first + Clean Architecture 简化版。
...
- `data/`: Data 层。SQLite、本地文件、导入导出、设置持久化。
...
Data 由 Application 或 UI 注入，不让 Domain 依赖 SQLite、Flutter 页面或平台实现。
